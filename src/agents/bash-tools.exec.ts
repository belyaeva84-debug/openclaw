...
export class ExecTool implements AgentTool<any, ExecToolDetails> {
  public readonly name = "exec";
  public readonly label = "exec";
  public readonly description =
    "Execute shell commands with background continuation. Use yieldMs/background to continue later via process tool. Use pty=true for TTY-required commands (terminal UIs, coding agents).";
  public readonly parameters = execSchema;

  private readonly defaultBackgroundMs: number;
  private readonly allowBackground: boolean;
  private readonly defaultTimeoutSec: number;
  private readonly defaultPathPrepend: string[];
  private readonly safeBins: Set<string>;
  private readonly notifyOnExit: boolean;
  private readonly notifySessionKey?: string;
  private readonly approvalRunningNoticeMs: number;
  private readonly agentId?: string;

  constructor(private readonly defaults?: ExecToolDefaults) {
    this.defaultBackgroundMs = clampWithDefault(
      defaults?.backgroundMs ?? readEnvInt("PI_BASH_YIELD_MS"),
      10_000,
      10,
      120_000,
    );
    this.allowBackground = defaults?.allowBackground ?? true;
    this.defaultTimeoutSec =
      typeof defaults?.timeoutSec === "number" && defaults.timeoutSec > 0
        ? defaults.timeoutSec
        : 1800;
    this.defaultPathPrepend = normalizePathPrepend(defaults?.pathPrepend);
    this.safeBins = resolveSafeBins(defaults?.safeBins);
    this.notifyOnExit = defaults?.notifyOnExit !== false;
    this.notifySessionKey = defaults?.sessionKey?.trim() || undefined;
    this.approvalRunningNoticeMs = resolveApprovalRunningNoticeMs(
  ...
  public execute = async (
    ...
    if (ctx.host === "node") {
      return this.handleNodeHost(ctx);
    }

    if (ctx.host === "gateway") {
      const approvalResult = await this.handleGatewayHost(ctx, onUpdate);
      if (approvalResult) {
        return approvalResult;
      }
    }

    return this.executeProcess(ctx, signal, onUpdate);
  };

  // oxlint-disable-next-line typescript/no-explicit-any
  private async buildContext(args: any): Promise<ExecContext> {
    const params = args as ExecContext["params"];
    ...
    if (!this.allowBackground && (backgroundRequested || yieldRequested)) {
      warnings.push("Warning: background execution is disabled; running synchronously.");
    }
    const yieldWindow = this.allowBackground
      ? backgroundRequested
        ? 0
        : clampWithDefault(
            params.yieldMs ?? this.defaultBackgroundMs,
            this.defaultBackgroundMs,
            10,
            120_000,
          )
      : null;
    const elevatedDefaults = this.defaults?.elevated;
    ...
    if (elevatedRequested) {
      if (!elevatedDefaults?.enabled || !elevatedDefaults.allowed) {
        const runtime = this.defaults?.sandbox ? "sandboxed" : "direct";
        const gates: string[] = [];
        const contextParts: string[] = [];
        const provider = this.defaults?.messageProvider?.trim();
        const sessionKey = this.defaults?.sessionKey?.trim();
        if (provider) {
    ...
    const configuredHost = this.defaults?.host ?? "sandbox";
    ...
    const configuredSecurity =
      this.defaults?.security ?? (host === "sandbox" ? "deny" : "allowlist");
    const requestedSecurity = normalizeExecSecurity(params.security);
    let security = minSecurity(configuredSecurity, requestedSecurity ?? configuredSecurity);
    if (elevatedRequested && elevatedMode === "full") {
      security = "full";
    }
    const configuredAsk = this.defaults?.ask ?? "on-miss";
    ...
    const sandbox = host === "sandbox" ? this.defaults?.sandbox : undefined;
    const rawWorkdir = params.workdir?.trim() || this.defaults?.cwd || process.cwd();
    ...
    applyPathPrepend(env, this.defaultPathPrepend);

    const effectiveTimeout =
      typeof params.timeout === "number" ? params.timeout : this.defaultTimeoutSec;
  ...
  private async handleNodeHost(ctx: ExecContext): Promise<AgentToolResult<ExecToolDetails>> {
    const { params, security, ask, warnings, workdir, env } = ctx;
    const approvals = resolveExecApprovals(this.agentId, { security, ask });
    ...
    const boundNode = this.defaults?.node?.trim();
    ...
    if (nodeEnv) {
      applyPathPrepend(nodeEnv, this.defaultPathPrepend, { requireExisting: true });
    }
    ...
    const invokeTimeoutMs = Math.max(
      10_000,
      (typeof params.timeout === "number" ? params.timeout : this.defaultTimeoutSec) * 1000 + 5_000,
    );
    const buildInvokeParams = (
      ...
      ({
        ...
        params: {
          ...
          agentId: this.agentId,
          sessionKey: this.defaults?.sessionKey,
    ...
    if (requiresAsk) {
      ...
      const noticeSeconds = Math.max(1, Math.round(this.approvalRunningNoticeMs / 1000));
      ...
      void (async () => {
        ...
        try {
          ...
          emitExecSystemEvent(
            ...
            { sessionKey: this.notifySessionKey, contextKey },
          );
          return;
        }
  ...
  private async handleGatewayHost(
    ctx: ExecContext,
    _onUpdate?: (partialResult: AgentToolResult<ExecToolDetails>) => void,
  ): Promise<AgentToolResult<ExecToolDetails> | null> {
    const { params, security, ask, warnings, workdir, env, bypassApprovals } = ctx;
    if (bypassApprovals) {
      return null;
    }
      ...
      void (async () => {
        ...
        if (decision === "deny") {
          ...
          if (hostSecurity === "allowlist") {
            for (const segment of allowlistEval.segments) {
              ...
              if (pattern) {
                addAllowlistEntry(approvals.file, this.agentId, pattern);
              }
            }
          }
        }
        ...
        try {
          run = await runExecProcess({
            ...
            usePty: params.pty === true && !ctx.sandbox,
            warnings,
            maxOutput: ctx.maxOutput,
            pendingMaxOutput: ctx.pendingMaxOutput,
            notifyOnExit: false,
            scopeKey: this.defaults?.scopeKey,
            sessionKey: this.notifySessionKey,
            timeoutSec: ctx.effectiveTimeout,
          });
        } catch {
          emitExecSystemEvent(
        ...
        const summary = output
          ...
          : `Exec finished (gateway id=${approvalId}, session=${run.session.id}, ${exitLabel})`;
        emitExecSystemEvent(summary, { sessionKey: this.notifySessionKey, contextKey });
      })();
  ...
  private async executeProcess(
    ...
    const { params, warnings, workdir, env, sandbox, containerWorkdir, effectiveTimeout } = ctx;
    ...
    return new Promise<AgentToolResult<ExecToolDetails>>((resolve, reject) => {
      ...
      if (this.allowBackground && ctx.yieldWindow !== null) {
        if (ctx.yieldWindow === 0) {
          onYieldNow();
        } else {
